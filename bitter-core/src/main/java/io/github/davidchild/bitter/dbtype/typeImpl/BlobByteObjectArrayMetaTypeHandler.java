...
public class BlobByteObjectArrayMetaTypeHandler extends TypeHanderDealBase<Byte[]> {

  @Override
  public void setNonNullParameter(PreparedStatement ps, int i, Byte[] parameter, MetaType jdbcType)
      throws SQLException {
    ByteArrayInputStream bis = new ByteArrayInputStream(MetaByteArrayUtils.convertToPrimitiveArray(parameter));
  ...
  private Byte[] getBytes(Blob blob) throws SQLException {
    ...
    if (blob != null) {
      returnValue = MetaByteArrayUtils.convertToObjectArray(blob.getBytes(1, (int) blob.length()));
    }
    return returnValue;
  }
}
