...
public class FindQuery<T extends BaseModel> extends BaseQuery implements IWhereQuery<FindQuery<T>,T>, IColumnQuery<FindQuery<T>,T>, IOrderQuery<FindQuery<T>,T> {
    ...
    public FindQuery<T> setSize(Integer size) {
        ...
        ((ExecuteParBagSelect) executeParBag).setTopSize(size);
        return this;
    }

    public int findCount() throws SQLException {
        ExecuteParBagCount countBag = new ExecuteParBagCount();
        countBag.setWhereCondition(((ExecuteParBagSelect)executeParBag).getWhereCondition());
